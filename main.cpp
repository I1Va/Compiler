...
const char AST_DOTCODE_OUTPATH[] = "./testing_space/AST_tree.txt";
const char AST_IMG_OUTPATH[] = "./testing_space/AST_img.png";
const char ASM_CODE_OUTPATH[] = "./testing_space/code.asm";

int main() {
    ast_tree_t ast_tree    =  {}; ast_tree_ctor(&ast_tree);
    str_storage_t *storage =  str_storage_t_ctor(STR_FUNCS_FRONTEND_CHUNK_SIZE);

    if (!FrontEnd_make_AST(&ast_tree, &storage, HIGH_LEVEL_CODE_PATH, AST_DOTCODE_OUTPATH, AST_IMG_OUTPATH)) {
